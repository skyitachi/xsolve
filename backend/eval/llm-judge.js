...
2. tutoring_strategy（1-5）：辅导策略是否恰当（引导而非代答、难度适配、循序渐进、**结合学生在草稿上的演算过程给出针对性反馈**）。5=策略优秀，1=策略很差。
...
【学生草稿（手写演算的自动识别结果，可能误读；无草稿时为空）】
${turn.scratch_ocr ? String(turn.scratch_ocr).slice(0, 500) : ((turn.scratch_strokes || 0) > 0 ? `（有 ${turn.scratch_strokes} 笔手写，未识别）` : '（本轮学生没有动笔）')}
